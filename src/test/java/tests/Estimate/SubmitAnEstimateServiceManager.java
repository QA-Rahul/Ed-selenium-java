...
public class SubmitAnEstimateServiceManager extends TestDriverActions {
    ...
    public void Test_001_gotoServiceBoardScreen() throws InterruptedException, FileNotFoundException {
        if(LoginActions.environmentName.contains("EC")) {
            loginPage.loginWithValidCredentials(appProp8.getProperty("username"),appProp8.getProperty("password"));
        } else if(LoginActions.environmentName.contains("EU")){
            loginPage.loginWithValidCredentials(appProp21.getProperty("username"),appProp21.getProperty("password"));
        }else if(LoginActions.environmentName.contains("NA")) {
            loginPage.loginWithValidCredentials(appProp36.getProperty("username"),appProp36.getProperty("password"));
...
//        submitanestimateservicemanager.clickOnIssueButton();
        submitanestimateservicemanager.clickOnAddPartButton();
