...
public class AddaFabricatedPartToaClosedWorkorder extends TestDriverActions {
    ...
    public void Test_001_gotoCompleteRo() throws InterruptedException, IOException, FileNotFoundException {
        ...
        if(loginActions.environmentName.contains("EC")) {
            loginPage.loginWithValidCredentials(appProp17.getProperty("username"), appProp17.getProperty("password"));
        } else if(loginActions.environmentName.contains("EU")) {
            loginPage.loginWithValidCredentials(appProp30.getProperty("username"), appProp30.getProperty("password"));
