...
public class CounterSale extends TestDriverActions {
    ...
    public void goto_countersaleManager_screen() throws FileNotFoundException, InterruptedException {
        if(LoginActions.environmentName.contains("EC")){
            loginPage.loginWithValidCredentials(appProp10.getProperty("username"),appProp10.getProperty("password") );
        } else if(LoginActions.environmentName.contains("NA")){
            loginPage.loginWithValidCredentials(appProp37.getProperty("username"),appProp37.getProperty("password") );
