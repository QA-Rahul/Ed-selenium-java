...
public class CreatePart extends TestDriverActions {
    ...
    public void gotoPartsMaster() throws FileNotFoundException, InterruptedException {
        if(loginActions.environmentName.contains("EC")) {
            loginPage.loginWithValidCredentials(appProp4.getProperty("username"), appProp4.getProperty("password"));
        } else if(loginActions.environmentName.contains("EU")){
            loginPage.loginWithValidCredentials(appProp18.getProperty("username"), appProp18.getProperty("password"));
        } else if(loginActions.environmentName.contains("NA")){
            loginPage.loginWithValidCredentials(appProp33.getProperty("username"), appProp33.getProperty("password"));
